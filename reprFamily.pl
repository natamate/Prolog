...
% jakie s¹ osoby w bazie?
istnieje(X).

% jakie s¹ dzieci w bazie?
dziecko(X).

% pokazaæ pensje wszystkich osób
istnieje(X),pensja(X,P).

% jakie dzieci urodzi³y siê w 1979r.?
dziecko(X),data_urodzenia(X,data(_,_,1979)).

% znaleŸæ wszystkie ¿ony, które pracuj¹
zona(X), pensja(X,P), P > 500.

% znaleŸæ osoby urodzone przed 1950 r, których pensja jest niemniejsza ni¿ 3000
 istnieje(X), data_urodzenia(X,data(_,_,Rok)), Rok < 1950, pensja(X,P), P >= 3000.
